...
<head>
    ...
    <title>Hizmetli</title>
    <link rel="stylesheet" href="../styleview.css">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-T3c6CoIi6uLrA9TneNEoa7RxnatzjcDSCmG1MXxSR1GAsXEV/Dwwykc2MPK8M2HN" crossorigin="anonymous">
</head>
<body>
    <h2 class="text-primary text-center bg-dark">Hizmetli Formu</h2>
...
try {
    ...
    echo "<table border='1'>
            <tr>
                ...
                <th>Hizmetli Ad</th>
                <th>Hemşrie Soyad</th>
                <th>Hizmetli Telefon</th>
                <th>Hizmetli Adres</th>
            </tr>";

    
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
        echo "<tr>
                <td>{$row['hizmetli_tc']}</td>
                <td>{$row['hizmetli_ad']}</td>
                <td>{$row['hizmetli_soyad']}</td>
                <td>{$row['hizmetli_telefon']}</td>
                <td>{$row['hizmetli_adres']}</td>
              </tr>";
    }
